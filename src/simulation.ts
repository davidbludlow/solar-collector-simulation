...
const ahj = -0.0150 * (dpipe_mm ** 2) + 1.40 * dpipe_mm + 0.510; // Penetration Depth Parameter a
...
let hIo = (ahj / 1000) * (Rio ** (-bhj)); // Initial Penetration Depth
...
let T2Top = new Array(tEND).fill(0);
let T2Bot = new Array(tEND).fill(0);
let T2 = new Array(tEND).fill(0);
let Co = new Array(tEND).fill(0);
let TIo = Array.from({ length: tEND }, () => new Array(101).fill(0));
let h = Array.from({ length: tEND }, () => new Array(101).fill(0));
let Ri = new Array(tEND).fill(0);
let hj = new Array(tEND).fill(0);

for (let i = 0; i < tEND; i++) {
    T2Top[i] = mfp * Tc * t[i] + mf * (Th - Tc) * (t[i] - Tau1 * Math.exp(t[i] / Tau1)) + CT2;
...
for (let i = 1; i < tEND; i++) {
    if (hj[i] <= Co[i]) { // Determine when thermocline passes jet penetration depth
        n = i - 1;
        break;
    }
}
...
const to = t[n] + ((hj[n] - Co[n]) * (t[n + 1] - t[n])) / ((Co[n + 1] - Co[n]) - (hj[n + 1] - hj[n]));
...
// ----------------------------------------------------------------------
// ----------------------------------------------------------------------
let TaveS = new Array(tEND).fill(0);
for (let i = 0; i < tEND; i++) {
    TaveS[i] = 0.5 * (TI[i] + Tc);
}
let time = new Array(tEND).fill(0);
time[1] = delt;

let Tth = Array.from({ length: tEND }, () => new Array(N).fill(0));
Tth[0][0] = Tc;

for (let i = 1; i < tEND; i++) {
    for (let j = 0; j < N; j++) {
        if (x[j] > C[i]) { // Below Thermocline
            Tth[i][j] = TaveS[i] + (Tc - TaveS[i]) * Math.erf((x[j] - C[i]) / Math.sqrt(4 * alfa * time[i]));
        } else { // Above Thermocline
            Tth[i][j] = (Tc + TI[i]) - (TaveS[i] + (Tc - TaveS[i]) * Math.erf((C[i] - x[j]) / Math.sqrt(4 * alfa * time[i])));
        }
    }

    // Determine Thermocline Thickness at the end of the time step
    let Wth = 2 * Math.sqrt(4 * alfa * time[i]) * Math.erfinv(1 + 0.001 * (Tc / (Tc - TaveS[i])));

    // Determine Time for profile with temperatures of next time step to obtain the same thermocline thickness
    let tnew = (1 / (4 * alfa)) * ((Wth / (2 * Math.erfinv(1 + 0.001 * (Tc / (Tc - TaveS[i + 1]))))) ** 2);
